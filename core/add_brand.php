...
if($title!=''){
  ...
  $id=$db->lastInsertId();?>
  window.top.window.$('#brand').append('<div id="l_<?php echo$id;?>" class="row"><div class="col-12 col-md-6"><div class="form-row"><input id="title<?php echo$id;?>" type="text" value="<?php echo$title;?>" readonly></div></div><div class="col-12 col-md-5"><div class="form-row"><input id="url<?php echo$id;?>" type="text" value="<?php echo$url;?>" readonly></div></div><div class="col-12 col-md-1"><div class="form-row"><?php echo$icon!=''?'<a data-fancybox="lightbox" href="'.$icon.'"><img id="thumbimage" src="'.$icon.'" alt="Thumbnail"></a>':'<img id="thumbimage" src="core/images/noimage-sm.jpg" alt="No Image">';?><form target="sp" action="core/purge.php"><input name="id" type="hidden" value="<?php echo$id;?>"><input name="t" type="hidden" value="choices"><button class="trash" data-tooltip="tooltip" data-title="Delete" type="submit" aria-label="Delete"><?php svg('trash');?></button></form></div></div></div>');
<?php }else
	echo'window.top.window.toastr["error"]("A Title wasn`t entered");';
echo'</script>';
