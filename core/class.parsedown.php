...
class Parsedown {
  ...
  protected function linesElements(array $lines) {
    ...
    foreach ($lines as $line) {
      if (chop($line) === '') {
        if (isset($CurrentBlock))$CurrentBlock['interrupted'] = (isset($CurrentBlock['interrupted']) ? $CurrentBlock['interrupted'] + 1 : 1);
        continue;
      }
      ...
      if (isset($this->BlockTypes[$marker])) {
        foreach ($this->BlockTypes[$marker] as $blockType)$blockTypes []= $blockType;
      }
      foreach ($blockTypes as $blockType) {
        ...
        if (isset($Block)) {
          ...
          if ( ! isset($Block['identified'])) {
            if (isset($CurrentBlock))$Elements[] = $this->extractElement($CurrentBlock);
            $Block['identified'] = true;
          }
          if ($this->isBlockContinuable($blockType))$Block['continuable'] = true;
          $CurrentBlock = $Block;
          continue 2;
        }
      }
      if (isset($CurrentBlock) and $CurrentBlock['type'] === 'Paragraph')$Block = $this->paragraphContinue($Line, $CurrentBlock);
      if (isset($Block))$CurrentBlock = $Block;
      else{
        if (isset($CurrentBlock))$Elements[] = $this->extractElement($CurrentBlock);
  ...
  protected function extractElement(array $Component) {
    if ( ! isset($Component['element'])) {
      if (isset($Component['markup']))$Component['element'] = array('rawHtml' => $Component['markup']);
      elseif (isset($Component['hidden']))$Component['element'] = array();
    }
  ...
  protected function blockList($Line, array $CurrentBlock = null){
    ...
    if (preg_match('/^('.$pattern.'([ ]++|$))(.*+)/', $Line['text'], $matches)){
      ...
      if ($name === 'ol'){
        ...
        if ($listStart !== '1'){
          if (isset($CurrentBlock) and $CurrentBlock['type'] === 'Paragraph' and ! isset($CurrentBlock['interrupted']))return;
  ...
  protected function blockListComplete(array $Block){
    if (isset($Block['loose'])){
      foreach ($Block['element']['elements'] as &$li){
        if (end($li['handler']['argument']) !== '')$li['handler']['argument'] []= '';
      }
    }
    return $Block;
  }
  ...
  protected function blockMarkup($Line){
    if($this->markupEscaped||$this->safeMode)return;
  ...
  protected function blockMarkupContinue($Line,array $Block){
    if(isset($Block['closed'])||isset($Block['interrupted']))return;
    $Block['element']['rawHtml'].= "\n".$Line['body'];
    return$Block;
  }
  ...
  protected function blockTable($Line,array $Block=null){
    if(!isset($Block)||$Block['type']!=='Paragraph'||isset($Block['interrupted']))return;
    if(strpos($Block['element']['handler']['argument'],'|')===false&&strpos($Line['text'], '|')===false&&strpos($Line['text'],':')===false||strpos($Block['element']['handler']['argument'],"\n")!==false)return;
  ...
  protected function lineElements($text,$nonNestables=array()){
    ...
    while($excerpt=strpbrk($text,$this->inlineMarkerList)){
      ...
      foreach($this->InlineTypes[$marker] as $inlineType){
        ...
        if(isset($Inline['position'])&&$Inline['position']>$markerPosition)continue;
  ...
  protected function inlineEmphasis($Excerpt){
    ...
    $marker=$Excerpt['text'][0];
    if($Excerpt['text'][1]===$marker&&preg_match($this->StrongRegex[$marker],$Excerpt['text'],$matches))$emphasis='strong';
  ...
  protected function inlineImage($Excerpt){
    if(!isset($Excerpt['text'][1])||$Excerpt['text'][1]!=='[')return;
  ...
  protected function inlineMarkup($Excerpt){
    if($this->markupEscaped||$this->safeMode||strpos($Excerpt['text'],'>')===false)return;
  ...
  protected function inlineUrl($Excerpt){
    if($this->urlsLinked!==true||!isset($Excerpt['text'][2]) or $Excerpt['text'][2]!=='/')return;
  ...
  protected function element(array $Element){
    ...
    if($hasContent){
      ...
      elseif(isset($Element['element']))$markup.=$this->element($Element['element']);
        ...
        if(!$permitRawHtml)$markup.=self::escape($text,true);else$markup.=$text;
      }
  ...
  protected function li($lines){
    ...
    if(!in_array('',$lines)&&isset($Elements[0])&&isset($Elements[0]['name'])&&$Elements[0]['name']==='p')unset($Elements[0]['name']);
    return $Elements;
  }
  ...
  protected function sanitiseElement(array $Element){
    ...
    if(!empty($Element['attributes'])){
      foreach($Element['attributes'] as $att=>$val){
        if(!preg_match($goodAttribute,$att))unset($Element['attributes'][$att]);elseif(self::striAtStart($att,'on'))unset($Element['attributes'][$att]);
      }
    }
    return$Element;
  }
  ...
  protected static function striAtStart($string,$needle){
    $len=strlen($needle);
    if($len>strlen($string))return false;else return strtolower(substr($string,0,$len))===strtolower($needle);
  }
