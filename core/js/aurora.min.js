...
function itemstoggle(source,act) {
  ...
  if(act=='read'){
    for(var i=0, n=checkboxes.length;i<n;i++){
      checkboxes[i].checked=false;
      if(checkboxes[i].dataset.status=='read'){
        checkboxes[i].checked=true;
      }
    }
    $('#itemchecker').prop("checked",true);
  ...
  if(act=='unread'){
    for(var i=0, n=checkboxes.length;i<n;i++){
      checkboxes[i].checked=false;
      if(checkboxes[i].dataset.status=='unread'){
        checkboxes[i].checked=true;
      }
    }
  }
  if(act=='important'){
    importants=document.getElementsByClassName('messages-important');
    for(var i=0, n=checkboxes.length;i<n;i++){
      checkboxes[i].checked=false;
...
function toggleBookingView(){
	$('#calendar-view,#list-view').toggleClass('d-none');
...
function selectTheme(t){
  ...
  document.cookie='admintheme=;expires=Thu, 01 Jan 1970 00:00:01 GMT;';
  var theme=$('body').attr('data-theme');
	if(t=='light'||t=='dark'){
		Cookies.set('admintheme',t,{expires:14});
    $('body').attr('data-theme',t);
    $('.theme-'+t).removeClass('d-none');
	}else if(t=='system'){
    if(t=='system'){
    ...
    Cookies.set('admintheme','light',{expires:14});
    $('body').attr('data-theme','light');
...
function toggleContentView(){
	$('#contentview').toggleClass('list');
  $('.contentview .i').toggleClass('d-none');
  document.cookie='contentview=;Expires=Thu, 01 Jan 1970 00:00:01 GMT;';
	if($('#contentview').hasClass('list')){
		Cookies.set('contentview','list',{expires:14});
	}else{
		Cookies.set('contentview','cards',{expires:14});
	}
	return false;
}
function toggleAccountView(){
	$('#accountview').toggleClass('list');
  $('.accountview .i').toggleClass('d-none');
  document.cookie='accountview=;Expires=Thu, 01 Jan 1970 00:00:01 GMT;';
	if($('#accountview').hasClass('list')){
		Cookies.set('accountview','list',{expires:14});
	}else{
		Cookies.set('accountview','cards',{expires:14});
	}
	return false;
}
...
$(document).on(
  ...
  ".quickeditbtn",function(){
    ...
    $('#quickedit'+qeid).toggleClass('qe-open').toggleClass('swing-in-top-fwd');
    if($('#quickedit'+qeid).hasClass('qe-open')){
      $('#l_'+qeid).toggleClass('quickedit-border');
      $('#quickedit'+qeid).html('<div class="row"><div class="col-12 p-3"><div class="spinner"><div class="sk-chase black"><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div><div class="sk-chase-dot"></div></div></div></div></div>');
      $.ajax({
        type:"GET",
...
$(document).on(
  "click",
  ".sidebardropdownbtn",function(){
    var sdid=$(this).data('sdid');
    $(this).find('.i').toggleClass('d-none');
    $('#sidebardropdown'+sdid).toggleClass('d-none').toggleClass('swing-in-top-fwd');
  }
);
$(document).on(
  "click",
  ".widgetdropdownbtn",function(){
    var sdid=$(this).data('sdid');
    $(this).find('.i').toggleClass('d-none');
    $('#widgetdropdown'+sdid).toggleClass('d-none').toggleClass('swing-in-top-fwd');
  }
);
$(document).on(
  "click",
  ".close-widget",function(){
    ...
    $.ajax({
      ...
      $('#l_'+id).addClass('zoom-out');
			setTimeout(function(){
        $('#l_'+id).remove();
        if($('#widgets').children().length){
        } else {
...
function statusSet(id,t,c,status){
  ...
  if($('#qestatus'+id).length > 0){
    ...
    $('#l_'+id).removeClass('delete');
    $('#l_'+id).removeClass('warning');
    $('#l_'+id).removeClass('info');
    if(status=='delete'){
      $('#l_'+id).addClass('delete');
      $('#untrash'+id).removeClass('d-none');
      $('#delete'+id).addClass('d-none');
      $('#purge'+id).removeClass('d-none');
    }else{
...
function more(t,c,b){
  $('.load-more').html('<div class="col-12 m-3 text-center btn"><i class="i i-spin">spinner</i></div>');
	$.ajax({
		type:"GET",
...
function trackertoblacklist(ip){
  $.ajax({
    type:"GET",
    url:"core/trackertoblacklist.php",
    data:{
      ip:ip
    }
  }).done(function(msg){
    if(msg!='error'){
      $('[data-ip="'+ip+'"] .tracker-status').html('<span class="badger badge-dark">Blacklisted</span>');
      $('[data-btnip="'+ip+'"]').css({'display':'none'});
    }else{
      console.log("Issue adding to Blacklist and Updating tracker Status!");
    }
  });
}
function purge(id,t,c){
	...
	$.ajax({
		...
		if(t=='clearip'){
			$('[data-ip="'+id+'"]').addClass('remove-item');
			setTimeout(function(){$('[data-ip="'+id+'"]').remove();},500);
		}else if(t=='iplist'||t=='logs'&&id==0){
			$('#l_'+t).addClass('remove-item');
			setTimeout(function(){$('#l_'+t).remove();},500);
		}else if(t=='media'){
			$('#mi_'+id).addClass('remove-item');
			setTimeout(function(){$('#mi_'+id).remove();},500);
    }else if(t=='choices'){
      $('#l_'+id).addClass('remove-item');
      setTimeout(function(){$('#l_'+id).remove();},500);
    }else if(t=='widgets'){
      $('#li_'+id).addClass('remove-item');
      setTimeout(function(){$('#li_'+id).remove();},500);
    }else if(t=='modules'){
      $('#modules_'+id).addClass('remove-item');
      setTimeout(function(){$('#modules_'+id).remove();},500);
    }else if(t=='module_questions'){
      $('#questions_'+id).addClass('remove-item');
      setTimeout(function(){$('#questions_'+id).remove();},500);
			...
			$('#l_'+t).addClass('remove-item');
			setTimeout(function(){$('#l_'+t).remove();},500);
    }else if(id!=0&&t=='playlist'){
      $('#pi_'+id).addClass('remove-item');
			setTimeout(function(){$('#pi_'+id).remove();},500);
    }else if(id!=0&&t=='courseTrack'){
      $('#student_'+id).addClass('remove-item');
			setTimeout(function(){$('#student_'+id).remove();},500);
    }else{
			$('#l_'+id).addClass('remove-item');
      $('#quickedit_'+id).addClass('remove-item');
			setTimeout(function(){
        $('#l_'+id).remove();
        $('#quickedit'+id).remove();
      },500);
...
function coverUpdate(id,t,c,da){
    ...
    var c2=c.toLowerCase();
		if(da==''){
			$('#'+c).val('');
			$('#'+c2+'image').attr('src','core/images/noimage-sm.jpg');
//      $('#'+c+'image')[0].parentNode.remove();
		}else{
			if(imgsrc==''){
				$('#'+c2+'image').attr('src',da);
//        $('#'+c+'image')[0].parentNode.remove();
        $('#'+c2+'image').wrap('<a data-fslightbox="'+c+'" href="'+da+'"></a>');
			}
		}
	})
}
...
var phrases=[" A cold one","A few stubbies short of a six-pack","A few sandwiches short of a picnic","Accadacca","Ace ","Aerial pingpong ","Ambo ","Amber fluid ","and","also","Ankle biter ","Apples ","Arvo ","As busy as a ","As cross as a","As cunning as a","As dry as a","As stands out like ","Aussie Rules Footy ","Aussie salute","Avo ","Avos ","Back of Bourke ","Bail ","Bail out ","Banana bender","Barbie","Barbie ","Barrack ","Bastard ","Bastards ","Bathers ","Battler ","Bazza","Bail up ","Beauty ","Better than a ham sandwich","Better than a kick up the backside ","Big Smoke ","Bikie","Billabong ","Billy ","Bingle ","Bities","Bitzer","Bizzo ","Blimey ","Blind ","Bloke","bloody","Bloody ","Bloody oath ","Bloody ripper ","Blow in the bag ","Blowie ","Bludger ","Blue ","Bluey","Boardies","Bodgy ","Bog standard ","Bogan","Bogged ","Boil-over ","Bonza","Bonzer ","Boogie board ","Booze","Booze bus","Boozer ","Bottle-o ","Bottlo","Bounce ","Bradman","Brass razoo","Brekkie ","Brickie ","Brisvegas ","Brizzie","Brolly","Bruce","Brumby ","Buck's night ","Buckley's chance ","Budgie smugglers ","Buggered","Built like a","Bull bar ","Bundy ","Bunyip ","Burk","Burk ","Bush","Bush bash ","Bush oyster ","Bush telly ","Bushie","Bushman's handkerchief ","Bushranger ","Butcher ","Cab Sav ","Catcus ","Cactus mate","Cane toad ","Captain Cook ","Cark it ","Carry on like a pork chop ","Carrying on like a pork chop ","Chewie ","Chin wag ","Chock a Block ","Chokkie ","Chook ","Chrissie ","Christmas ","Chuck a sickie","Chuck a spaz","Chuck a yewy","Chunder ","Ciggies","Ciggy","Clacker ","Cleanskin","Clucky ","Coathanger ","Cobber ","Cockie ","Cockie","Coldie","Come a ","Compo","Cooee","Cook ","Coppers ","Corker ","Counter meal","Crack the shits ","Cracker","Cranky ","Cream ","Crikey ","Crook ","Crow eaters ","Cubby house ","Cut lunch ","Cut lunch commando","Cut snake","Dag","Daks ","Damper ","Dardy ","Dead horse ","Deadset ","decent nik","Defo ","Dero ","Devo ","Digger ","Dill ","Dinky-di","Dinky-di ","Dipstick ","Divvy van ","Do the Harry ","Dob","Dog's balls","Dog's breakfast ","Dog's eye","Dole bludger ","Donga ","Donger ","Doovalacky ","Down Under ","Drongo","Dropkick ","Dry as as dead dingo's donga","Dunny","Dunny ","Dunny rat","Durry ","Esky","Eureka","Facey ","Fair crack of the whip","Fair dinkum ","Fair go ","Fair go, mate","Fair suck of the sauce bottle","Fairy floss ","Feral","Few roos loose in the top paddock","Figjam ","Fisho ","Flake ","flamin'","Flanno","Flanny","Flat out","Flat out like a","Flick","Fly wire ","Footy ","Ford","Fossicker","Franger ","Freckle","Fremantle Doctor ","Freo ","Frog in a sock","Frothy ","Fruit loop ","Full blown ","Full boar ","Furthy","G'day","Galah","Galah ","Garbo","Get a dog up ya","Give it a burl ","Gnarly","Gobful","Gobsmacked ","Going off ","Good oil ","Good onya ","Gone Walkabout ","Goon","Goon Bag","Grab us a ","Greenie ","Grog ","Grouse ","Grundies ","Gutful of ","gutta","Gyno ","Hard yakka ","Have a Captain Cook ","Have a go, you mug","Head like a dropped pie","heaps","Heaps","He hasn't got a","he's dreaming ","He's got a massive","His blood's worth bottling ","Hit the frog and toad","Hit the turps","Holden","Holy dooley ","Home grown ","Hoon ","Hooroo","Hottie ","how","Icy pole","Iffy ","Ironman","It'll be","Jackaroo","Jillaroo ","Joey","Jug","Jumbuck ","Kelpie","Ken Oath","Kero ","Kindie ","Knackered ","Knickers","Knock ","Lappy","Larrikin","Laughing gear ","leg it","Lets get some","Lets throw a","Like a madwoman's shit ","Lippy ","Lizard drinking","Lollies","Longneck ","Loose cannon ","Lurk ","Maccas","Mad as a","Manchester","Mates","mate","Mate ","Mate's rates","Metho","Mickey Mouse Mate","Middy","Milk bar ","Mokkies","Mongrel ","Moolah","Mozzie","Mullet","Muster ","my","Nah, yeah ","Nipper","no dramas","No dramas","no worries","No worries","No worries, mate, she'll be right","No Wucka's","No wuckin' furries ","No-hoper ","Not my bowl of rice ","Not my cup of tea ","Nuddy","Ocker ","Off chops ","Offsider ","oi","Oldies","On the cans ","One for the road","Onya bike ","Op shop","Outback","Paddock ","Parma","Pash","Pav ","Pelican","Piece of Piss","Piker","pinga","Pint ","Piss Off","Piss Up","Pissed","Pissed Off","Plonk ","Pokies","Porky ","Postie","Pot ","Pozzy ","Pretty spiffy ","Prezzy ","Pub","Pull the wool over your eyes ","Pull the wool over their eyes ","Put a sock in it ","Quid","Rack off ","Rage on ","Rapt ","Ratbag ","Reckon","Rego","Rellie","Rello ","Ridgy-didge","Rip snorter","Ripper ","Road train ","Roadie ","Rock up ","Rollie","Roo ","Roo bar ","Root Rat","Ropeable ","Rort ","Rotten","Rubbish ","Runners","Sandgropper ","Sandgroppers ","Sanger ","Schooner","Scratchy","Servo","Shag on a rock","Shark Biscuit ","Shazza","Shazza got us some ","She'll be apples ","She'll be right","Shoey","Shoot Through","Sickie","Sheila","Sheila ","Shonky","Show pony ","Sick","Sickie","Six of one, half a dozen of the other","Skeg ","Skite ","Skull ","Slab ","Slabs","Slacker","slappa ","Slaps","Sleepout ","Smokes","Smoko ","Snag ","Sook ","Spag bol ","Spewin' ","Spit the dummy","Squizz ","Stands out like a","Stickybeak","Stoked ","Stonkered ","Straight to the pool room","Straya ","Strewth","Strides ","Struth","Stubby","Stubby holder ","Stuffed ","Sunnies ","Suss",,"Suss it out","Swag ","Swagger","Taking the piss ","Tell him he's dreaming ","Tell your story walkin'","Ten Clicks away","Thingo ","Thongs ","Throw a shrimp on the barbie ","Throw-down ","Thunderbox ","Tickets on yourself ","Tinny","to ","Toads ","Too right","Top Bloke","Top End ","Tosser ","Trackie dacks ","Trackies","Trackies ","Tradie","Trent from punchy ","Troppo","Truckie ","True blue ","Tucker ","Tucker-bag","Turps ","Two pot screamer","Two up ","U-IE ","Ugg","Ugg boots ","Uluru","unit ","Up the duff ","Up Yours","Up Yourself","Ute","Ute ","VB","Vee dub","Veg out ","Vinnie's","Waggin' school","Waratah","Watch out for the","Waazoo ","What's crackin' ","What's the John Dory?","Whinge ","We're going ","when","where","whit","Wobbly ","Woop Woop","Wouldn't piss on them if they're on fire","Wrap your laughing gear 'round that","Wuss","ya","Yabber ","Yakka","Yobbo ","You little ripper","Your Shout","Yous"
];
...
document.addEventListener('keydown',e=>{
  if(e.ctrlKey && e.key==='s'){
    e.preventDefault();
    if(document.querySelector('.saveall').className.match('btn-danger')){
      document.querySelector('.saveall').click();
    }
  }
});
function seoLink(field,tab){
