...
document.addEventListener('click',function(event){
  if(event.target.closest(".nolightbox")){
    event.preventDefault();
    var img=event.target.closest(".nolightbox").getAttribute("href");
    var cap=event.target.closest(".nolightbox").getAttribute("data-caption");
    document.getElementById("figureimage").removeAttribute("srcset");
    document.getElementById("figureimage").src=img;
    document.getElementById("figcaption").innerHTML=cap;
  }
...
function acceptShift(id,uid){
  document.querySelector("#shift"+id).classList.remove('d-none');
  fetch("core/updateshift.php",{
    method:"POST",
    headers:{"Content-type":"application/x-www-form-urlencoded;charset=UTF-8"},
    body:"id="+id+"&uid="+uid
  }).then(function(response){
    ...
    if(data!=`error`){
      document.querySelector("#shift"+id).classList.add('alert-success');
      document.querySelector("#shift"+id).innerHTML=data;
    }else{
      document.querySelector("#shift"+id).classList.add('alert-danger');
      if(data===''){
        document.querySelector("#shift"+id).innerHTML=`Error: `+data;
      }else{
        document.querySelector("#shift"+id).innerHTML=`There was an issue returning the output data!`;
      }
    }
  });
}
