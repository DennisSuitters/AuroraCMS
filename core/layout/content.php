      ...
      <?php }?>
                <div class="form-row justify-content-end">
                  ...
                  <div class="btn-group">
                    ...
                    <a class="btn" data-tooltip="left" href="<?= URL.$settings['system']['admin'].'/content/settings';?>" role="button" aria-label="Content Settings"><i class="i">settings</i></a>
                    <?=(isset($args[1])&&$args[1]!=''&&$user['options'][0]==1?'<a class="btn add" data-tooltip="left" href="'.URL.$settings['system']['admin'].'/add/'.$args[1].'" role="button" aria-label="Add '.ucfirst($args[1]).'"><i class="i">add</i></a>':'');?>
                  </div>
                </div>
              </div>
            </div>
      ...
      <?php while($r=$s->fetch(PDO::FETCH_ASSOC)){
        ...
        if($r['contentType']!='testimonials'){
          ...
          if(isset($h1[1]))$seoerrors++;
            ...
            <article class="card zebra mx-2 mb-0 overflow-visible card-list" id="l_<?=$r['id'];?>" data-content="<?=$r['contentType'].' '.$r['title'];?>">
        ...
        <?php if($r['contentType']=='inventory'){?>
                <select class="stock status" data-tooltip="tooltip" onchange="update('<?=$r['id'];?>','content','stockStatus',$(this).val(),'select');" aria-label="Stock Status">
        ...
        <?='<small class="text-muted d-block" id="rank'.$r['id'].'">Available to '.($r['rank']==0?'<span class="badger badge-secondary">Everyone</span>':'<span class="badger badge-'.rank($r['rank']).'">'.ucwords(str_replace('-',' ',rank($r['rank']))).'</span> and above').'</small>';?>
                ...
                <div class="card-footer">
                  ...
                  <?=(isset($cnt['cnt'])&&$cnt['cnt']>0?'<a class="btn views'.($sccc>0?' add':'').'" href="'.URL.$settings['system']['admin'].'/content/edit/'.$r['id'].'#tab1-5" role="button" data-tooltip="tooltip" aria-label="'.$sccc.' New Comments">'.$cnt['cnt'].' <i class="i">comments</i></a>':'');?>
                  ...
                  echo$rss['cnt']>0?'<button class="btn views" data-tooltip="tooltip" aria-label="'.$rss['cnt'].' Sales this Month">'.$rss['cnt'].' <i class="i">shipping</i></button>':'';?>
                    ...
                    <div class="btn-toolbar float-right" role="toolbar">
                      <div class="btn-group" role="group">
                        ...
                        <?php if($user['options'][0]==1){?>
                          ...
                          <button class="btn quickeditbtn" data-qeid="<?=$r['id'];?>" data-qet="content" data-tooltip="tooltip" aria-label="Open/Close Quick Edit Options"><i class="i">chevron-down</i><i class="i d-none">chevron-up</i></button>
                        <?php }?>
                      </div>
                    </div>
                  </div>
