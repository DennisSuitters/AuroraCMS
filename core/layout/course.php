      ...
      <div class="card mt-3 border-radius-0 bg-transparent border-0 overflow-visible">
        ...
        <div class="col-12 col-sm-6 text-right">
          <div class="form-row justify-content-end">
            ...
            <div class="btn-group">
              <button class="btn contentview" data-tooltip="left" aria-label="View Courses as Cards or List" onclick="toggleContentView();return false;"><i class="i<?=($_COOKIE['contentview']=='list'?' d-none':'');?>">list</i><i class="i<?=($_COOKIE['contentview']=='cards'?' d-none':'');?>">cards</i></button>
              <a class="btn" href="<?= URL.$settings['system']['admin'].'/course/settings';?>" role="button" data-tooltip="left" aria-label="Course Settings"><i class="i">settings</i></a>
              <?=($user['options'][0]==1?'<a class="btn add" href="'.URL.$settings['system']['admin'].'/add/course" role="button" data-tooltip="left" aria-label="Add Course"><i class="i">add</i></a>':'');?>
            </div>
          </div>
        </div>
      </div>
      ...
      <?php while($r=$s->fetch(PDO::FETCH_ASSOC)){?>
        <article class="card zebra mx-2 mb-0 overflow-visible card-list" data-content="course<?=' '.$r['title'];?>" id="l_<?=$r['id'];?>">
          ...
          <div class="card-header overflow-visible mt-0 pt-0 line-clamp">
            <a href="<?= URL.$settings['system']['admin'].'/course/edit/'.$r['id'];?>" data-tooltip="tooltip" aria-label="Edit <?=$r['title'];?>"><?= $r['thumb']!=''&&file_exists($r['thumb'])?'<img src="'.$r['thumb'].'"> ':'';echo$r['title'];?></a>
              ...
              <div id="controls_<?=$r['id'];?>">
                <div class="btn-toolbar float-right" role="toolbar">
                  <div class="btn-group" role="group">
                    ...
                    <a class="btn" href="<?= URL.$settings['system']['admin'];?>/course/edit/<?=$r['id'];?>" role="button" data-tooltip="tooltip"<?=$user['options'][1]==1?' aria-label="Edit"':' aria-label="View"';?>><i class="i"><?=$user['options'][1]==1?'edit':'view';?></i></a>
