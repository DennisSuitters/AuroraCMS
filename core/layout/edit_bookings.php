  ...
  <section id="content">
      ...
      <div class="content-title">
        <div class="content-title-heading">
          ...
          <div class="content-title-actions">
            ...
            <?php }?>
            <a class="btn" href="#" data-tooltip="tooltip" aria-label="Print Booking" onclick="$('#sp').load('core/print_booking.php?id=<?=$r['id'];?>');return false;"><?= svg2('print');?></a>
            <?php if($user['options'][0]==1||$user['options'][2]==1){?>
              <a class="btn" href="#" data-tooltip="tooltip" aria-label="Email Booking" onclick="$('#sp').load('core/email_booking.php?id=<?=$r['id'];?>');return false;"><?= svg2('email-send');?></a>
            <?php }?>
    ...
    <div class="container-fluid p-0">
      <div class="card border-radius-0 px-4 py-3 overflow-visible">
        ...
        <label class="col-form-label col-12 col-sm-1 px-0" id="bookingService" for="rid"><?=$user['rank']>899?'<a class="permalink" href="'.URL.$settings['system']['admin'].'/bookings/edit/'.$r['id'].'#bookingService" data-tooltip="tooltip" aria-label="PermaLink to Booking Service Selector">&#128279;</a>':'';?>Service</label>
        <div class="form-row">
...
<?php $sql=$db->query("SELECT `id`,`contentType`,`code`,`title`,`assoc` FROM `".$prefix."content` WHERE `bookable`='1' AND `title`!='' AND `status`='published' AND `internal`!='1' ORDER BY `code` ASC,`title` ASC");
        ...
        <?php if($config['bookingAgreement']!=''){?>
          <div class="row mt-3">
            ...
            <input id="agreementCheck" data-dbid="<?=$r['id'];?>" data-dbt="content" data-dbc="agreementCheck" data-dbb="0" type="checkbox"<?=$r['agreementCheck'][0]==1?' checked aria-checked="true"':' aria-checked="false"';?>>
