  ...
  $gst=number_format((float)$gst,2,'.','');
            ...
            if($r['contentType']=='inventory'){?>
              <div class="row">
                ...
                <div class="col-12 col-sm-4 pr-md-3">
                  <label id="<?=$r['contentType'];?>Points" for="quantity"><?=$user['rank']>899?'<a class="permalink" href="'.URL.$settings['system']['admin'].'/content/edit/'.$r['id'].'#'.$r['contentType'].'Points" data-tooltip="tooltip" aria-label="PermaLink to '.ucfirst($r['contentType']).' Points Value Field">&#128279;</a>':'';?>Points Value</label>
                  <div class="form-row">
              ...
              if($sm->rowCount()>0){
                while($rm=$sm->fetch(PDO::FETCH_ASSOC)){
                  if(file_exists('media/md/'.basename($rm['file'])))
                    ...
                    $thumb=ADMINNOIMAGE;?>
                  <div id="mi_<?=$rm['id'];?>" class="card stats col-6 col-md-3 m-1">
                    <?php if($user['options'][1]==1){?>
...
<?php /*
            ...
            <div class="form-row">
              ...
              <?=$user['options'][1]==1?'<button class="save" id="saveseoKeywords" data-dbid="seoKeywords" data-style="zoom-in" data-tooltip="tooltip" aria-label="Save">'.svg2('save').'</button>':'';?>
            </div>
