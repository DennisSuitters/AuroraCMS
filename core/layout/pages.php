          ...
          <table class="table-zebra">
            <thead>
              <tr>
                ...
                <th class="col text-center">Views<?=$user['options'][1]==1?' <button class="trash" data-tooltip="tooltip" aria-label="Clear All Page Views" onclick="$(`[data-views=\'views\']`).text(`0`);purge(`0`,`pageviews`);">'.svg2('eraser').'</button>':'';?></th>
...
<?php if($r['cover']!=''){
  ...
  if(file_exists('media/lg/'.$imgcheck)&&file_exists('media/sm/'.$imgcheck)){
    echo'<a data-fancybox="media" data-caption="'.$r['title'].($r['fileALT']!=''?'<br>ALT: '.$r['fileALT']:'<br>ALT: <span class=text-danger>Edit the ALT Text for SEO (Will use above Title instead)</span>').'" href="media/lg/'.$imgcheck.'"><img class="img-rounded" style="max-width:32px;height:32px;" src="media/sm/'.$imgcheck.'" alt="'.$r['title'].'"></a>';
  }
}?>
                  </td>
                  <td class="align-top">
                      ...
                      <div class="d-block ml-5">
                        <?php while($rm=$sm->fetch(PDO::FETCH_ASSOC)){?>
                          <div class="zebra border-bottom position-relative" style="position:relative;">
                            ...
                            <?='<br><small class="text-muted">Available to '.($rm['rank']==0?'Everyone':ucfirst(rank($rm['rank'])).' and above').'</small>';?>
                            <span style="position:absolute;top:0;right:0;" id="controls_<?=$rm['id'];?>" role="group">
                              <?=$user['options'][0]==1?'<button class="btn btn-sm trash align-top" data-tooltip="tooltip" aria-label="Clear" onclick="$(`#views'.$rm['id'].'`).text(`0`);update(`'.$rm['id'].'`,`menu`,`views`,`0`);"><span id="views'.$rm['id'].'">'.$rm['views'].'</span></button>':$rm['views'];?>
                              <span class="i" style="width:24px;">&nbsp;</span>
                              <a class="btn btn-sm" href="<?= URL.$settings['system']['admin'].'/content/edit/'.$rm['id'];?>"<?=$user['options'][1]==1?' data-tooltip="tooltip" role="button" aria-label="Edit"':' data-tooltip="tooltip" role=button aria-label="View"';?>"><?=$user['options'][1]==1?svg2('edit'):svg2('view');?></a>
                              <?=$user['options'][0]==1&&$rm['contentType']=='page'?'<button class="btn btn-sm trash" data-tooltip="tooltip" aria-label="Delete" onclick="purge(\''.$rm['id'].'\',\'menu\');">'.svg2('trash').'</button>':'';?>
                              <span class="i">&nbsp;</span>
                            </span>
                          </div>
                        <?php }?>
                  ...
                  <td class="text-center"><?= ucfirst($r['menu']);?></td>
                  ...
                  <td class="text-center" id="menuactive0<?=$r['id'];?>">
                  ...
                  <td><?php svg('drag','orderhandle');?></td>
                </tr>
              <?php }?>
