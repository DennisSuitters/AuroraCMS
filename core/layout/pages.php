 ...
 * @changes    v0.0.17 Add display to indicate access level for pages.
 * @changes    v0.0.17 Reconfigure page listing display.
...
if($args[0]=='add'){
  $ti=time();
  $q=$db->prepare("INSERT IGNORE INTO `".$prefix."menu` (`uid`,`mid`,`login_user`,`title`,`seoTitle`,`file`,`contentType`,`schemaType`,`menu`,`active`,`ord`,`eti`) VALUES (:uid,'0',:login_user,:title,'','page','page','Article','other','0',:ord,:eti)");
  $q->execute([
    ':uid'=>$user['id'],
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-menu">
          <div class="btn-group" role="group">
            <?php echo$user['options'][0]==1?'<a class="btn btn-ghost-normal add" href="'.URL.$settings['system']['admin'].'/pages/add" data-tooltip="tooltip" data-placement="left" data-title="Add" aria-label="Add">'.svg2('add').'</a>':'';?>
          </div>
        </li>
      </ol>
            ...
            <table class="table table-responsive-sm table-sm table-hover">
              <thead>
                <tr>
                  <th class="col">
                    <div class="row">
                      <div class="col-12 col-sm-6">Title</div>
                      <div class="col-12 col-sm-6">Submenu Title</div>
                    </div>
                  </th>
                  <th class="col-1 text-center">Menu</th>
                  <th class="col text-center"><span class="d-inline">Views&nbsp;</span><?php echo$user['options'][1]==1?'<button class="btn btn-secondary btn-xs trash d-inline" onclick="$(`[data-views=\'views\']`).text(`0`);purge(`0`,`pageviews`);" data-tooltip="tooltip" data-title="Clear All" aria-label="Clear All">'.svg2('eraser').'</button>':'';?></th>
                  ...
                  <tr id="l_<?php echo$r['id'];?>" class="item subsortable">
                    <td class="align-top">
                      <div class="row">
                        ...
                        if($sm->rowCount()>0){?>
                          <div class="col-12 col-sm-6 align-top border-left">
                            <?php while($rm=$sm->fetch(PDO::FETCH_ASSOC)){?>
                              <div  id="s_<?php echo$rm['id'];?>" class="sub zebra border-bottom position-relative">
                                ...
                                <span id="controls_<?php echo$rm['id'];?>" class="float-right align-top" style="position:absolute;top:3px;right:0;" role="group">
                                  <?php echo$user['options'][0]==1?'<button class="btn btn-secondary dark btn-sm trash align-top" onclick="$(`#views'.$rm['id'].'`).text(`0`);update(`'.$rm['id'].'`,`menu`,`views`,`0`);" data-tooltip="tooltip" data-title="Clear" aria-label="Clear">'.svg2('eraser').' <span id="views'.$rm['id'].'">'.$rm['views'].'</span></button>':$rm['views'];?>
                                  <?php echo$r['contentType']!='index'?'<label class="switch switch-label switch-success"><input type="checkbox" id="active'.$rm['id'].'" class="switch-input" data-dbid="'.$rm['id'].'" data-dbt="menu" data-dbc="active" data-dbb="0"'.($rm['active']==1?' checked aria-checked="true"':' aria-checked="false"').($user['options'][1]==1?'':' disabled').'><span class="switch-slider" data-checked="on" data-unchecked="off"></span></label>':'';?>
                                  <a class="btn btn-secondary btn-sm align-top" href="<?php echo URL.$settings['system']['admin'].'/pages/edit/'.$rm['id'];?>"<?php echo$user['options'][1]==1?' data-tooltip="tooltip" data-title="Edit" aria-label="Edit"':' data-tooltip="tooltip" data-title="View" aria-label="View"';?>"><?php echo$user['options'][1]==1?svg2('edit'):svg2('view');?></a>
                                  <?php echo$user['options'][0]==1&&$rm['contentType']=='page'?'<button class="btn btn-secondary btn-sm trash align-top" onclick="purge(\''.$rm['id'].'\',\'menu\')" data-tooltip="tooltip" data-title="Delete" aria-label="Delete">'.svg2('trash').'</button>':'';?>
                                </span>
                              </div>
                            <?php }?>
                          </div>
                    ...
                    <td class="align-top text-center"><?php echo ucfirst($r['menu']);?></td>
                    <td class="align-top text-center">
                      <?php echo$user['options'][1]==1?'<button class="btn btn-secondary trash" onclick="$(`#views'.$r['id'].'`).text(`0`);updateButtons(`'.$r['id'].'`,`menu`,`views`,`0`);" data-tooltip="tooltip" data-title="Clear" aria-label="Clear">'.svg2('eraser').'&nbsp;&nbsp;<span id="views'.$r['id'].'" data-views="views">'.$r['views'].'</span></button>':$r['views'];?>
                    </td>
                    <td class="align-top text-center">
                    ...
                    <td id="controls_<?php echo$r['id'];?>" class="align-top">
                      <div class="btn-group float-left">
                        <a class="btn btn-secondary" href="<?php echo URL.$settings['system']['admin'].'/pages/edit/'.$r['id'];?>"<?php echo$user['options'][1]==1?' data-tooltip="tooltip" data-title="Edit" aria-label="Edit"':' data-tooltip="tooltip" data-title="View" aria-label="View"';?>"><?php echo$user['options'][1]==1?svg2('edit'):svg2('view');?></a>
                        <?php echo$user['options'][0]==1&&$r['contentType']=='page'?'<button class="btn btn-secondary btn-sm trash" onclick="purge(\''.$r['id'].'\',\'menu\')" data-tooltip="tooltip" data-title="Delete" aria-label="Delete">'.svg2('trash').'</button>':'';?>
                      </div>
                    </td>
                  </tr>
                <?php }?>
