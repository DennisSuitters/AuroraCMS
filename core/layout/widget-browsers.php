...
<?php
  $sb=$db->prepare("SELECT `browser`,COUNT(DISTINCT `ip`) AS `cnt` FROM `".$prefix."tracker` WHERE `ti`>:sD GROUP BY (`browser`) ORDER BY `cnt` DESC");
  $sb->execute([':sD'=>$currentMonthStart - 1]);
  while($rb=$sb->fetch(PDO::FETCH_ASSOC)){
    if(stristr('unknown',$rb['browser']))continue;
    ?>
      <div class="col-9 py-1"><?=$rb['browser'];?></div><div class="col-3 py-1 text-right"><?= number_format($rb['cnt']);?></div>
<?php }?>
    </div>
  </div>
</div>
