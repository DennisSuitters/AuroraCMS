  ...
  <div class="alert widget widget-limit m-3 p-0" id="widgetbrowsers<?=$rw['id'];?>">
    ...
    <?php if($cnt>5){?>
      <div class="row widget-more">
        <button class="widget-more-btn btn-ghost" data-tooltip="tooltip" aria-label="Show/Hide Extra Items"  onclick="$(`#widgetbrowsers<?=$rw['id'];?>`).toggleClass('widget-limit');$(`.widgetbrowsers`).toggleClass('d-none');return false;"><i class="i widgetbrowsers">down</i><i class="i widgetbrowsers d-none">up</i></button>
      </div>
    <?php }?>
  </div>
</div>
