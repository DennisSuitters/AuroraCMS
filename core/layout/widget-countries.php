  ...
  <div class="alert widget widget-limit m-3 p-0" id="widgetcountries<?=$rw['id'];?>">
    ...
    <?php if($cnt>5){?>
      <div class="row widget-more">
        <button class="widget-more-btn" data-tooltip="tooltip" aria-label="Show/Hide Extra Items"  onclick="$(`#widgetcountries<?=$rw['id'];?>`).toggleClass('widget-limit');$(`.widgetcountries`).toggleClass('d-none');return false;"><i class="i widgetcountries">down</i><i class="i widgetcountries d-none">up</i></button>
      </div>
    <?php }?>
  </div>
</div>
