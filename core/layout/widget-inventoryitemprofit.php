 ...
 *
 * @category   Administration - Dashboard - Widget - Inventory Item Expenses
 * @package    core/layout/widget-inventoryitemprofit.php
...
if(isset($r['contentType'])&&$r['contentType']!=''){
  if($r['contentType']=='inventory'||$r['contentType']=='service'||$r['contentType']=='course'){
    ...
    <div class="card bg-white">
      <canvas id="profitChart" style="min-height:225px;height:225px;max-height:225px;max-width:100%;"></canvas>
    </div>
    <?php
      $profit=$r['cost'] - $r['expense'];?>
    <script>
      var profitChartData={
        labels:[
          '<?= date("M",$times[1][0]);?>',
        ...
        datasets:[
          {
            label: 'This Year',
            data:[
              <?=$m1r['cnt']>0?$m1r['cnt'] * $profit:0;?>,
              <?=$m2r['cnt']>0?$m2r['cnt'] * $profit:0;?>,
              <?=$m3r['cnt']>0?$m3r['cnt'] * $profit:0;?>,
              <?=$m4r['cnt']>0?$m4r['cnt'] * $profit:0;?>,
              <?=$m5r['cnt']>0?$m5r['cnt'] * $profit:0;?>,
              <?=$m6r['cnt']>0?$m6r['cnt'] * $profit:0;?>,
              <?=$m7r['cnt']>0?$m7r['cnt'] * $profit:0;?>,
              <?=$m8r['cnt']>0?$m8r['cnt'] * $profit:0;?>,
              <?=$m9r['cnt']>0?$m9r['cnt'] * $profit:0;?>,
              <?=$m10r['cnt']>0?$m10r['cnt'] * $profit:0;?>,
              <?=$m11r['cnt']>0?$m11r['cnt'] * $profit:0;?>,
              <?=$m12r['cnt']>0?$m12r['cnt'] * $profit:0;?>
            ]
          },
          {
            label: 'Last Year',
            data:[
              <?=$my1r['cnt']>0?$my1r['cnt'] * $profit:0;?>,
              <?=$my2r['cnt']>0?$my2r['cnt'] * $profit:0;?>,
              <?=$my3r['cnt']>0?$my3r['cnt'] * $profit:0;?>,
              <?=$my4r['cnt']>0?$my4r['cnt'] * $profit:0;?>,
              <?=$my5r['cnt']>0?$my5r['cnt'] * $profit:0;?>,
              <?=$my6r['cnt']>0?$my6r['cnt'] * $profit:0;?>,
              <?=$my7r['cnt']>0?$my7r['cnt'] * $profit:0;?>,
              <?=$my8r['cnt']>0?$my8r['cnt'] * $profit:0;?>,
              <?=$my9r['cnt']>0?$my9r['cnt'] * $profit:0;?>,
              <?=$my10r['cnt']>0?$my10r['cnt'] * $profit:0;?>,
              <?=$my11r['cnt']>0?$my11r['cnt'] * $profit:0;?>,
              <?=$my12r['cnt']>0?$my12r['cnt'] * $profit:0;?>
            ]
          }
        ]
      }
      var profitChartCanvas=$('#profitChart').get(0).getContext('2d');
      var profitChartData=$.extend(true,{},profitChartData);
      ...
      $profcnt=($m1r['cnt'] +
        ...
        $m11r['cnt'] +
        $m12r['cnt']);
      $earnings=$profcnt * $r['cost'];
      $profit=$earnings - ($profcnt * $r['expense']);?>

      $('#analytics-earnings').html('&dollar;<?=parseCurrency($earnings);?>');
      $('#analytics-profit').html('&dollar;<?=parseCurrency($profit);?>');
    </script>
<?php }
}
