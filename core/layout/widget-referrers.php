  ...
  <div class="alert widget widget-limit m-3 p-0" id="widgetreferrers<?=$rw['id'];?>">
    ...
    <?php if($cnt>5){?>
      <div class="row widget-more">
        <button class="widget-more-btn btn-ghost" data-tooltip="tooltip" aria-label="Show/Hide Extra Items"  onclick="$(`#widgetreferrers<?=$rw['id'];?>`).toggleClass('widget-limit');$(`.widgetreferrers`).toggleClass('d-none');return false;"><i class="i widgetreferrers">down</i><i class="i widgetreferrers d-none">up</i></button>
      </div>
    <?php }?>
  </div>
</div>
