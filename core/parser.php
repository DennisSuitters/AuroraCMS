...
if(preg_match('/<author>([\w\W]*?)<\/author>/',$parse)&&$view=='article'&&$r['uid']!=0)$parse=str_replace(['<author>','</author>'],'',$parse);
	...
	switch($print){
		...
		case'categories':
			if(isset($r['category_1'])&&$r['category_1']!=''){
				...
				if($r['category_3']!='')$parsing.=' / <a href="'.$view.'/'.urlencode(str_replace(' ','-',$r['category_1'])).'/'.urlencode(str_replace(' ','-',$r['category_2'])).'/'.urlencode(str_replace(' ','-',$r['category_3'])).'/">'.htmlspecialchars($r['category_3'],ENT_QUOTES,'UTF-8').'</a>';
				if($r['category_4']!='')$parsing.=' / <a href="'.$view.'/'.urlencode(str_replace(' ','-',$r['category_1'])).'/'.urlencode(str_replace(' ','-',$r['category_2'])).'/'.urlencode(str_replace(' ','-',$r['category_3'])).'/'.urlencode(str_replace(' ','-',$r['category_4'])).'/">'.htmlspecialchars($r['category_4'],ENT_QUOTES,'UTF-8').'</a>';
			}else$container=$parsing='';
			break;
		case'tags':
		...
		case'thumb':
			...
			if($r['thumb']!=''&&(file_exists($thumbfolder.DS.$thumbchk)||file_exists('..'.DS.'..'.DS.$thumbfolder.DS.$thumbchk)))$parsing.='<img src="'.$r['thumb'].'" alt="'.$r['title'].'">';
		...
		case'email':
			if($attribute=='author'){
				if($author['email'])$parsing.='<a href="mailto:'.$author['email'].'">'.($type=='icon'?'<'.$theme['settings']['icon_container'].' class="'.$class.'">'.frontsvg('libre-gui-email').'</'.$theme['settings']['icon_container'].'>':$author['email']).'</a>';
			}
			break;
		case'social':
			if($attribute=='author'){
