	...
	switch($print){
		...
		case'categories':
			if(isset($r['category_1'])&&$r['category_1']!=''){
				...
				if($r['category_2']!='')
					$parsing.=' / <a href="'.$view.'/'.urlencode(str_replace(' ','-',$r['category_1'])).'/'.urlencode(str_replace(' ','-',$r['category_2'])).'/'.(isset($_GET['theme'])?'?theme='.$_GET['theme']:'').'">'.htmlspecialchars($r['category_2'],ENT_QUOTES,'UTF-8').'</a>';
				if($r['category_3']!='')
					$parsing.=' / <a href="'.$view.'/'.urlencode(str_replace(' ','-',$r['category_1'])).'/'.urlencode(str_replace(' ','-',$r['category_2'])).'/'.urlencode(str_replace(' ','-',$r['category_3'])).'/'.(isset($_GET['theme'])?'?theme='.$_GET['theme']:'').'">'.htmlspecialchars($r['category_3'],ENT_QUOTES,'UTF-8').'</a>';
				if($r['category_4']!='')
					$parsing.=' / <a href="'.$view.'/'.urlencode(str_replace(' ','-',$r['category_1'])).'/'.urlencode(str_replace(' ','-',$r['category_2'])).'/'.urlencode(str_replace(' ','-',$r['category_3'])).'/'.urlencode(str_replace(' ','-',$r['category_4'])).'/'.(isset($_GET['theme'])?'?theme='.$_GET['theme']:'').'">'.htmlspecialchars($r['category_4'],ENT_QUOTES,'UTF-8').'</a>';
			}else
				$container=$parsing='';
			break;
		case'tags':
