...
if($timecode!=''){
  ...
  if($config['formMinTime']!=0){
    if($timecheck < $config['formMinTime'])
      $not=['spammer'=>true,'target'=>'reset','element'=>'','action'=>'replace','class'=>'not alert alert-danger','text'=>'Woah, too quick, blacklisted!','reason'=>'Reset Password Form filled in too quickly! ('.$config['formMinTime'].' seconds)'];
  }
  if($config['formMaxTime']!=0){
    if($timecheck > ($config['formMaxTime']*60))
      $not=['spammer'=>true,'target'=>'reset','element'=>'','action'=>'replace','class'=>'not alert alert-danger','text'=>'Way too long to fill out a form!','reason'=>'Reset Password Form Exceeded time allowed! ('.$config['formMaxTime'].' minutes)'];
  }
}
if($config['reCaptchaServer']!=''){
  if(isset($_POST['g-recaptcha-response'])){
    ...
    if(!$captcha)
      $not=['spammer'=>false,'target'=>'reset','element'=>'','action'=>'replace','class'=>'not alert alert-danger','text'=>'reCaptcha failed, maybe the setup is wrong!','reason'=>''];
    else{
      ...
      if($responseKeys["success"])
        $not=['spammer'=>true,'target'=>'','element'=>'','action'=>'none','class'=>'','text'=>'','reason'=>'Reset Password Form reCaptcha Failed'];
    }
  }
}
  ...
  if($act=='reset_password'){
    if($config['php_options'][3]==1&&$config['php_APIkey']!=''){
      ...
      if($h->hasRecord()==1||$h->isSuspicious()==1||$h->isCommentSpammer()==1)
        $not=['spammer'=>true,'target'=>'reset','element'=>'div','action'=>'replace','class'=>'not alert alert-danger','text'=>'Your IP is classified as Malicious and has been added to our Blacklist, for more information visit the Project Honey Pot website.','reason'=>'Reset Password Form found Blacklisted IP via Project Honey Pot'];
    }
    if($_POST['fullname'.$hash]==''){
      ...
      if($config['spamfilter'][0]==1&&$not['spammer']==false){
        ...
        if($result)
          $not=['spammer'=>true,'target'=>'resetemail','element'=>'div','action'=>'replace','class'=>'not mt-3 alert alert-danger','text'=>'The data entered into the Form fields has been detected by our Filters as Spammy.','reason'=>'Reset Password Form, Spam Detected via Form Field Data.'];
      }
      if($not['spammer']==false){
        ...
        if($s->rowCount()>0){
          ...
          $msg=str_replace([
        	...
        	$mail->AltBody=$msg;
        	if($mail->Send())
            $not=['spammer'=>false,'target'=>'reset','element'=>'div','action'=>'replace','class'=>'not alert alert-success','text'=>'Check your Email!','reason'=>''];
          else
            $not=['spammer'=>false,'target'=>'reset','element'=>'div','action'=>'replace','class'=>'not alert alert-danger','text'=>'Problem Sending Email!','reason'=>''];
        }else
          $not=['spammer'=>false,'target'=>'resetemail','element'=>'div','action'=>'after','class'=>'not mt-3 alert alert-danger','text'=>'No Account Found!','reason'=>''];
      }
    }else
      $not=['spammer'=>false,'target'=>'resetemail','element'=>'div','action'=>'after','class'=>'not mt-3 alert alert-danger','text'=>'The Email provided is invalid!','reason'=>''];
