...
if($config['comingsoon'][0]==1||$config['maintenance'][0]==1){
  ...
  $s1=$db->query("SELECT `contentType`,`title`,`metaRobots`,`rank` FROM `".$prefix."menu` WHERE `active`='1' AND `contentType`!='settings' AND `contentType`!='comingsoon' AND `contentType`!='maintenance' AND `contentType`!='offline' AND `contentType`!='activate' AND `contentType`!='notification' ORDER BY `ord` ASC");
